...
export function LoteoCard({ development }: { development: Development }) {
 ...
 return (
 <article data-testid={'development-card-' + development.slug} className={'group overflow-hidden rounded-[2rem] border border-white/90 bg-white shadow-[0_30px_70px_-48px_rgba(15,23,42,0.25)] transition hover:-translate-y-1 hover:shadow-[0_38px_82px_-50px_rgba(15,23,42,0.28)]'}>
...
export function FilterBar(props: { search: string; location: string; status: string; surface: string; locations: string[]; onSearch: (value: string) => void; onLocation: (value: string) => void; onStatus: (value: string) => void; onSurface: (value: string) => void; }) {
 return (<div data-testid={'developments-filters'} className={'grid gap-4 rounded-[2rem] border border-white/90 bg-white/95 p-5 shadow-[0_24px_50px_-40px_rgba(15,23,42,0.24)] lg:grid-cols-4'}><label className={'space-y-2'}><span className={'text-xs font-semibold uppercase tracking-[0.18em] text-slate-500'}>Buscador</span><input data-testid={'filter-search'} value={props.search} onChange={(event) => props.onSearch(event.target.value)} placeholder={'Buscar por nombre'} className={'w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-300 focus:bg-white'} /></label><label className={'space-y-2'}><span className={'text-xs font-semibold uppercase tracking-[0.18em] text-slate-500'}>Ubicacion</span><select data-testid={'filter-location'} value={props.location} onChange={(event) => props.onLocation(event.target.value)} className={'w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-300 focus:bg-white'}><option value={'all'}>Todas</option>{props.locations.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label className={'space-y-2'}><span className={'text-xs font-semibold uppercase tracking-[0.18em] text-slate-500'}>Disponibilidad</span><select data-testid={'filter-status'} value={props.status} onChange={(event) => props.onStatus(event.target.value)} className={'w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-300 focus:bg-white'}><option value={'all'}>Todos los estados</option><option value={'disponible'}>Disponible</option><option value={'consultado'}>Consultado</option><option value={'reservado'}>Reservado</option><option value={'vendido'}>Vendido</option></select></label><label className={'space-y-2'}><span className={'text-xs font-semibold uppercase tracking-[0.18em] text-slate-500'}>Superficie</span><select data-testid={'filter-surface'} value={props.surface} onChange={(event) => props.onSurface(event.target.value)} className={'w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-sky-300 focus:bg-white'}><option value={'all'}>Cualquier rango</option><option value={'300-320'}>300 a 320 m2</option><option value={'321-340'}>321 a 340 m2</option><option value={'341-380'}>341 a 380 m2</option></select></label></div>);
}

export function ViewToggle({ value, onChange }: { value: 'mapa' | 'lista'; onChange: (value: 'mapa' | 'lista') => void; }) {
 return (<div data-testid={'view-toggle'} className={'inline-flex rounded-[1.35rem] border border-slate-200 bg-slate-50 p-1.5 shadow-[0_16px_34px_-28px_rgba(15,23,42,0.25)]'}><button data-testid={'view-toggle-mapa'} onClick={() => onChange('mapa')} className={'rounded-[1rem] px-5 py-2.5 text-sm font-semibold transition ' + (value === 'mapa' ? 'bg-white text-slate-950 ring-1 ring-slate-200 shadow-[0_12px_22px_-18px_rgba(15,23,42,0.28)]' : 'text-slate-600 hover:text-slate-900')}>Mapa</button><button data-testid={'view-toggle-lista'} onClick={() => onChange('lista')} className={'rounded-[1rem] px-5 py-2.5 text-sm font-semibold transition ' + (value === 'lista' ? 'bg-white text-slate-950 ring-1 ring-slate-200 shadow-[0_12px_22px_-18px_rgba(15,23,42,0.28)]' : 'text-slate-600 hover:text-slate-900')}>Lista</button></div>);
}
